...
class MainWindow(QtWidgets.QWidget):
    ...
    def on_snip(self, img):
        result = ocr_image_data(img, self.src_combo.currentData())
    ...
    def _is_repetitive_noise_text(self, text: str) -> bool:
        ...
        stretched_runs = len(re.findall(r"([a-z一-龯ぁ-んァ-ン])\1{2,}", compact))
        if stretched_runs >= 3:
            return True
        return False
                ...
                if self.memory_patch_worker:
                    try:
                        if self._is_recent_qlie_text(text):
                            accepted = bool(self.memory_patch_worker.update_mapping(text, trans))
                            if accepted:
                                self._write_debug_event("replace.memory_mapping_update", source=text, target=trans)
                            else:
                                self._write_debug_event(
                                    "replace.memory_mapping_skipped",
                                    source=text,
